...
const RepeatButton = (props) => {
  ...
  let className = "spin-button" + (disabled ? " disabled" : "");

  return (<a className={className} onClick={onClick} href="">Play!</a>)
}
...
export default class Dashboard extends React.Component {
  ...
  componentDidMount() {
    const { image_1, image_2, image_3 } = this.state;
    if (!image_1) {
      generateImage(1, this.getImage);
    }
    if (!image_2) {
      generateImage(2, this.getImage);
    }
    if (!image_3) {
      generateImage(3, this.getImage);
    }
  }

  handleClick() {
    this.setState({ runs: true, winner: null, balance: this.state.balance - 1, winLines: null, win: null });
    this.emptyArray();
  ...
  finishHandler = (value) => {
    ...
    if (Dashboard.matches.length === 3) {
      ...
      this.setState({ firstLine, secondLine, thirdLine, runs: false }, () => {
        this.resolveWin();
      });
    }
  }
      ...
      <div>
        ...
        {this.state.balance > 0 ?
          <div className="spinner-button">
            <RepeatButton onClick={this.handleClick} disabled={this.state.runs} />
          </div>
          : null}
